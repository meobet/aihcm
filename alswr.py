...
def RMSE(preds, truth):
    return np.sqrt(mean_squared_error(y_pred=preds, y_true=truth))
...
class ALS(object):
    ...
    def _init_item_ratings(self, ratings):
        item_ratings = pd.DataFrame(ratings).groupby(1)[2].mean()
        for i in range(self.n_item):
            if i in item_ratings.index:
                self.item_features_[i, 0] = item_ratings.loc[i]

    def fit(self, ratings, n_iters=50):
        if self.mean_rating_ is None:
            self.mean_rating_ = np.mean(ratings.take(2, axis=1))
        else:
            self._init_item_ratings(ratings)
